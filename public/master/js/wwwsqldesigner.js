...
const id = parseInt(location.search.substr(4,location.search.length));
if(isNaN(id)){
	location.href = ruta;
}
SQL.Designer = function() {
	SQL.Designer = this;
	this.socket = io.connect(ruta,{'forceNew':true});
	this.tables = [];
	this.relations = [];
...
SQL.Designer.prototype.addTable = function(name, x, y) {
	...
	//var data = {xml : this.toXML()}
	//this.socket.emit('servidor',data);
	return t;
}
