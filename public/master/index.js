...
$(document).ready(function () {
  ...
  function sendMessage() {
    var text = $input.val();
    messenger.send(text);
    var data = [{mensaje : text},{ origen : usuario}];
    d.socket.emit('servidor',data);
    $input.val('');
    $input.focus();
  }
  ...
  d.socket.on('mensajes', function(data){
    if(data[1].origen != usuario)
		  messenger.recieve(data[1].origen + ' : '+ data[0].mensaje);
	});
